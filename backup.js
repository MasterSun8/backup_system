...
async function createZipArchive(file) {
    try {
        ...
        fs.writeFile('/var/log/kopiaZapasowa.txt', (`Created ${file}successfullywith ${len}files on${todayDate()}`), { flag: 'a+' }, err => {})
    } catch (error) {
    }
}
...
try{
    ...
    fs.writeFile('/var/log/kopiaZapasowa.txt', ("/n access to some files in etc denied" + todayDate()), { flag: 'a+' }, err => {})
}

if(back.includes(etcFile)){
    createZipArchive(etcFile)}
else{
    fs.writeFile('/var/log/kopiaZapasowa.txt', ("/n no need for another etc backup" + todayDate()), { flag: 'a+' }, err => {})
}
...
try{
    ...
    fs.writeFile('/var/log/kopiaZapasowa.txt', ("/n access to some files in home denied" + todayDate()), { flag: 'a+' }, err => {})
}

if(back.includes(homeFile)){
    createZipArchive(homeFile)}
else{
    fs.writeFile('/var/log/kopiaZapasowa.txt', ("/n no need for another home backup" + todayDate()), { flag: 'a+' }, err => {})
}
...
if(back.includes(dbFile)){
    ...
    fs.writeFile('/var/log/kopiaZapasowa.txt', ("/n no need for another db backup" + todayDate()), { flag: 'a+' }, err => {})
}
